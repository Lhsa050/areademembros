...
<div class="max-w-2xl">
    ...
    <div class="bg-white rounded-xl shadow-sm border p-6 mb-6">
        <form method="POST" action="<?= url('/funnels/' . $funnel['id'] . '/access-levels/' . $level['id']) ?>" class="space-y-6">
            <?= csrf_field() ?>
            <div><label class="block text-sm font-medium text-gray-700 mb-2">Nome</label><input type="text" name="name" value="<?= e($level['name']) ?>" class="w-full px-4 py-2 border border-gray-200 rounded-lg" required></div>
            <div>
                <label class="block text-sm font-medium text-gray-700 mb-2">Senha</label>
                <div class="flex gap-2">
                    <input type="text" id="password" name="password" value="<?= e($level['password']) ?>" class="flex-1 px-4 py-2 border border-gray-200 rounded-lg uppercase" required>
                    <button type="button" onclick="generatePassword('simple')" class="bg-gray-100 text-gray-700 px-4 py-2 rounded-lg text-sm font-medium hover:bg-gray-200">Simples</button>
                    <button type="button" onclick="generatePassword('secure')" class="bg-gray-100 text-gray-700 px-4 py-2 rounded-lg text-sm font-medium hover:bg-gray-200">Segura</button>
                </div>
            </div>
            <div class="bg-blue-50 p-4 rounded-lg">
                <p class="text-xs text-blue-600 mb-2 font-medium">Parâmetro de URL</p>
            ...
            <div>
                ...
                <div class="space-y-2 max-h-60 overflow-y-auto border rounded-lg p-3">
                    ...
                    <label class="flex items-center gap-3 p-2 hover:bg-gray-50 rounded-lg cursor-pointer">
                        <input type="checkbox" name="products[]" value="<?= $product['id'] ?>" <?= in_array($product['id'], $selectedProducts) ? 'checked' : '' ?> class="w-4 h-4 text-blue-500 rounded">
    ...
    <div class="bg-red-50 rounded-xl p-4 border border-red-100">
        <form method="POST" action="<?= url('/funnels/' . $funnel['id'] . '/access-levels/' . $level['id'] . '/delete') ?>" onsubmit="return confirm('Remover?')">
            <?= csrf_field() ?>
            <button type="submit" class="bg-red-500 text-white px-4 py-2 rounded-lg text-sm font-medium hover:bg-red-600">Excluir</button>
        </form>
    </div>
</div>
<script>
